// js-problems-part2-practice-tasks

// Task-01: Find the lowest number in the array below.
// const heights2 = [167, 190, 120, 165, 137];

function lowestHeight(arrayValue){
    let low = arrayValue[0];
    for(const val of arrayValue){
        if(val < low){
            low = val;
        }
    }
    return low;
}

const heights2 = [167, 190, 120, 165, 137];

console.log(lowestHeight(heights2));

// Task-02: Find the friend with the smallest name.
// const heights = ['rahim', 'robin', 'rafi', 'ron', 'rashed'];

function smallestName(arrayValue){
    let small = arrayValue[0];
    for(const val of arrayValue){
        if(val.length < small.length){
            small = val;
        }
    }
    return small;
}

const heights = ['rahim', 'robin', 'rafi', 'ron', 'rashed'];

console.log(smallestName(heights));

// Task-03: Your task is to calculate the total budget required to buy electronics:
//     laptop = 35000 tk
//     tablet = 15000 tk
//     mobile = 20000 tk
// Write a JavaScript function named calculateElectronicsBudget that takes in the number of laptop, tablets, and mobile and returns the total money required.

function calculateElectronicsBudget(laptopQuantity, tabletQuantity, mobileQuantity){
    let laptop = 35000;
    let tablet = 15000;
    let mobile = 20000;
    let laptopCost = laptopQuantity * laptop;
    let tabletCost = tabletQuantity * tablet;
    let mobileCost = mobileQuantity * mobile;
    let totalCost = laptopCost + tabletCost + mobileCost;
    return totalCost;
}

console.log(calculateElectronicsBudget(1, 1, 1));

// Task-04: You are given an array of phone objects, each containing information about the model, brand, and price. Your task is to write a JavaScript function named findAveragePhonePrice that takes this array as input and returns the average price of phone.
// Input:
//     const phones = [
//         { model: "PhoneA", brand: "Iphone", price: 95000 },
//         { model: "PhoneB", brand: "Samsung", price: 40000 },
//         { model: "PhoneC", brand: "Oppo", price: 26000 },
//         { model: "PhoneD", brand: "Nokia", price: 35000 },
//         { model: "PhoneE", brand: "Iphone", price: 105000 },
//         { model: "PhoneF", brand: "HTC", price: 48000 },
//     ];

function findAveragePhonePrice(phones){
    let sum = 0;
    for(let ph of phones){
        sum = sum + ph.price;
    }
    let avg = sum / phones.length;
    return avg;
}

const phones = [
    { model: "PhoneA", brand: "Iphone", price: 95000 },
    { model: "PhoneB", brand: "Samsung", price: 40000 },
    { model: "PhoneC", brand: "Oppo", price: 26000 },
    { model: "PhoneD", brand: "Nokia", price: 35000 },
    { model: "PhoneE", brand: "Iphone", price: 105000 },
    { model: "PhoneF", brand: "HTC", price: 48000 },
];

console.log(parseFloat(findAveragePhonePrice(phones).toFixed(2)));

// Task-05: For each employee their current salary is calculated by multiplying yearly increment with experience then adding the result to the starting salary. Now calculate is the total salary has to be provided by the company in a month.

//  const employees = [
//     { name: "shahin", experience: 5, starting: 20000, increment: 5000 },
//     { name: "shihab", experience: 3, starting: 15000, increment: 7000 },
//     { name: "shikot", experience: 9, starting: 30000, increment: 1000 },
//     { name: "shohel", experience: 0, starting: 29000, increment: 4000 },
// ];

function totalSalary(infoArray){
    let currentTotalSalary = 0;
    for(const info of infoArray){
        currentTotalSalary = currentTotalSalary + info.starting + (info.experience * info.increment);
    }
    return currentTotalSalary;
}

const employees = [
    { name: "shahin", experience: 5, starting: 20000, increment: 5000 },
    { name: "shihab", experience: 3, starting: 15000, increment: 7000 },
    { name: "shikot", experience: 9, starting: 30000, increment: 1000 },
    { name: "shohel", experience: 0, starting: 29000, increment: 4000 },
];

console.log(totalSalary(employees));